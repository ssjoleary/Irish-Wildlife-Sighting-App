...
public abstract class ImgurUploadTask extends AsyncTask<Void, Void, String> {
    ...
    private ContentResolver mActivity;
    private Uri mImageUri;  // local Uri to upload

    public ImgurUploadTask(Uri imageUri, ContentResolver activity) {
    ...
    protected String doInBackground(Void... params) {
        InputStream imageIn;
        try {
            imageIn = mActivity.openInputStream(mImageUri);
        } catch (FileNotFoundException e) {
            Log.e(LocationUtils.APPTAG, "could not open InputStream", e);
            return null;
        }
        ...
        try {
            ...
            if (conn.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                Log.d(LocationUtils.APPTAG, "responseCode=" + conn.getResponseCode());
                ...
                Log.d(LocationUtils.APPTAG, "error response: " + sb.toString());
                return null;
            }
        } catch (Exception ex) {
            Log.e(LocationUtils.APPTAG, "Error during POST", ex);
            return null;
        } finally {
            try {
                responseIn.close();
    ...
    protected String onInput(InputStream in) throws Exception {
        ...
        Log.d(LocationUtils.APPTAG, "new imgur url: http://imgur.com/" + id + " (delete hash: " + deletehash + ")");
        return id;
    }
